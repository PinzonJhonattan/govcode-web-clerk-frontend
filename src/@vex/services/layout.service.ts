...
export class LayoutService {
  ...
  private _quickpanelOpenSubject = new BehaviorSubject<boolean>(true);
  quickpanelOpen$ = this._quickpanelOpenSubject.asObservable();

  // private _sidenavOpenSubject = new BehaviorSubject<boolean>(false);
  private _sidenavOpenSubject = new BehaviorSubject<boolean>(true);
  ...
  getOpenSideNavOpen() {
    return this._sidenavOpenSubject.value;
  }

  toggleSidenav() {
    const currentValue = this._sidenavOpenSubject.value;
    this._sidenavOpenSubject.next(!currentValue);
    const currentValue2 = this._sidenavCollapsedSubject.value;
    this._sidenavCollapsedSubject.next(!currentValue2);
  }
