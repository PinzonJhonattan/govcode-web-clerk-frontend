...
@Component({
  ...
  templateUrl: './select-multiple-field.html',
  styleUrls: ['./select-multiple-field.scss']
})
export class SelectMultipleField extends DynamicFieldBaseComponentComponent implements OnInit, OnDestroy{
  dynamicProps : any;
  ...
  override ngOnInit() {
    ...
    this.fieldControl.valueChanges.subscribe(data => {
      const multiplesValuesSelected = data?.split(",") || [];
      const optionsChoosed = this.dynamicProps.options
        .filter(option => multiplesValuesSelected.includes(option.label))
        .map(optionChoosed => optionChoosed?.value);
      groupSection.get(this.fieldNameValue).setValue(optionsChoosed.join(","))
    })
  }
  getValuesSelect() {
