...
export class EditRoleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.manageRolesClerkService.getRoleById(this.roleClerkData.infoRole.id).subscribe({
      next: (data) =>{
  ...
  handleEditRole () {
    ...
    this.manageRolesClerkService.editRole(this.roleClerkData.infoRole.id, {
      ...
      next: (data) => {
        ...
        Swal.fire({
          position: "bottom-end",
          html: `<p>Rol editado exitosamente</p>`,
          timer: 2000,
      ...
      error: error => {
        ...
        this.messageError = 'Error: ' + error?.error?.message || 'Ha ocurrido un error al editar el rol intente de nuevo'
      }
    })
  }
